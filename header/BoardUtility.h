...
class Mover {
  public:
    ...
    int convertTo = 0;      // used to indicate Promotions:
...
std::vector<Mover>
Str2Moves(std::string &movesString); // From string of moves to vector of moves
Mover Move2Mover(const move &move);  // From low level representation to high
move Mover2Move(
    const Mover
        &mover); // From high level representation to low
                 // -----------------------------------------------------------
