...
class Engine {
  public:
    ...
    bool
        stopFlag; // Flag that can be set from the controller to stop the engine
    Board gameHistory[200];    // History of the game, consisting of boards.
    uint gameHistoryIndex = 0; // Index to the current board
    std::vector<bitboard>
        repetitionTable; // Table of zobrist hashes to check for repetitions
    ...
    void SetBoard(
        const Board &board); // Resets engine and sets board as current board
    ...
    void
    MakePermanentMove(const move &move); // Make a move that can not be undone
    void UndoLastMove();                 // Revert to previous position
    void UndoLastSimpleMove();           // Undo a simple move
    void GetLegalMoves(std::array<move, 256> &moveHolder, uint &moveHolderIndex,
                       bool capturesOnly); // Gets legalMoves, if capturesOnly
                                           // get legal captures
    ...
    // --------------------------------------------------------------------------------------------
    bitboard
    Perft(int depth); // Counts all leafs up to fixed depth of the current board
    // --------------------------------------------------------------------------------------------
    ...
    float
    Evaluate(); // Debug tool to return static evaluation of current position
    void HashTest(
        int depth); // Testing Zobrist Key generation
                    // --------------------------------------------------------------------------------------------
};
// ------------------------------------------------------------
