...
void Sort(std::array<T, SIZE> &target, std::array<R, SIZE> &comparer, int upTo,
          bool descending);
} // namespace MathUtility

template <typename T> MathUtility::Random<T>::Random(T seed) { state = seed; }

template <typename T> T MathUtility::Random<T>::Next() {
...
template <typename T, typename R, int SIZE>
void MathUtility::Sort(std::array<T, SIZE> &target,
                       std::array<R, SIZE> &comparer, int upTo,
                       bool descending) {
    T tmpT;
    R tmpR;
    if (descending) {
