...
Mover Str2Move(const std::string &moveString) {
    Mover move = Mover(Str2Coord(moveString.substr(0, 2)),
                       Str2Coord(moveString.substr(2, 2)));
...
std::vector<Mover> Str2Moves(std::string &movesString) {
    ...
    std::regex move_regex(
        "\\S+"); // create regex for chain of non space characters
    auto move_begin =
        std::sregex_iterator(movesString.begin(), movesString.end(),
                             move_regex);   // Get iterator of front
    auto move_end = std::sregex_iterator(); // Get Iterator of back
    for (std::sregex_iterator i = move_begin; i != move_end;
         i++) {                 // Loop over all matches
        std::smatch match = *i; // ?
...
move Mover2Move(const Mover &mover) {
    return Move(mover.startIndex, mover.targetIndex, mover.convertTo,
                mover.promotion);
}
// -----------------------------------------------------------
