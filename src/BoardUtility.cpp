...
Coord
Str2Coord (const std::string &coordString)
{
    return Coord (cols.find (coordString[0]), rows.find (coordString[1]));
}

std::string
Move2Str (const Mover &move)
{
    std::string tmp = "";
...
Mover
Str2Move (const std::string &moveString)
{
...
std::vector<Mover>
Str2Moves (std::string &movesString)
{
    std::vector<Mover> moves;
    // Courtesy of chatgbt
    std::regex move_regex ("\\S+"); // create regex for chain of non space characters
    auto move_begin = std::sregex_iterator (movesString.begin (), movesString.end (),
                                            move_regex); // Get iterator of front
...
std::string
Moves2Str (std::vector<move> &movesVector)
{
    std::string res = " ";
    for (uint i = 0; i < movesVector.size (); i++)
        {
            res += Move2Str (Move2Mover (movesVector[i])) + " ";
        }
    return res;
}

Mover
Move2Mover (const move &move)
{
