...
void Engine::Boot() {
    ...
    ComputeMasks();
    std::cerr << "Engine: Calling MoveGenerator::PreComputeMoves()"
              << std::endl;
    MoveGenerator::PreComputeMoves();
    std::cerr << "Engine: Calling ComputeHashes()" << std::endl;
    ComputeHashes();
    std::cerr << "Engine: Calling transposition::Tt(" << ttSize << ")"
              << std::endl;
...
void Engine::SetBoard(const Board &newBoard) {
    ...
    std::cerr << "Engine: Calling CurrentBoard().InitialiseColorBoards()"
              << std::endl;
...
void Engine::MakeMove(const move &move) {
    std::memcpy(&gameHistory[gameHistoryIndex + 1],
                &gameHistory[gameHistoryIndex], sizeof(Board));
    gameHistoryIndex++;
...
void Engine::UndoLastSimpleMove() { gameHistoryIndex--; }

void Engine::GetLegalMoves(std::array<move, 256> &moveHolder,
                           uint &moveHolderIndex, bool capturesOnly) {
    if (capturesOnly) {
...
bool Engine::IsCheck() {
    ...
    int kingIndex =
        BitScanForwards(currentBoard.pieceBoards[5 + colorIndex]) - 1;
...
float Engine::Evaluate() {
    ...
    return colorMultipliyer * Evaluation::StaticEvaluation(*this) /
           (float)pieceValues[0];
}
