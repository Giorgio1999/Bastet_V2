...
// Fen parsing
// -------------------------------------------------------------------
Board Fen2Position() { return Fen2Position(startingPosition); }
...
Board Fen2Position(const std::string &fen) {
    ...
    // Player to move
    std::string rights =
        fen.substr(fen.find_first_of(' ') + 1, fen.length() - 1);
    ...
    std::string enPassant =
        rights.substr(castling.length() + 3, rights.length() - 1);
    ...
    std::string nonReversibleMoves = rights.substr(
        castling.length() + enPassant.length() + 4, rights.length() - 1);
    nonReversibleMoves =
        nonReversibleMoves.substr(0, nonReversibleMoves.find_first_of(' '));
